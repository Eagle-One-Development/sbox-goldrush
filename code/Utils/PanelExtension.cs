...
public static class PanelExtension
{
	...
	public static void PositionAtCrosshair( this Panel panel, Entity pawn )
	{
		if ( !pawn.IsValid() )
			return;

		if ( pawn is not GoldRush.Player player )
			return;

		var baseAimRay = new Ray( player.EyePosition, player.BaseEyeRotation.Forward );

		var tr = Trace.Ray( baseAimRay, 2000.0f )
						.Size( 1.0f )
						.Ignore( player )
						.UseHitboxes()
						.Run();
